...
class FindClosestPortTest {
    ...
    void findPortClose() throws ParseException {
        ...
        if (!ReadFilesController.isUseBigFiles()) {
            ...
            assertEquals(p, findClosestPort.findPortClose(BuildTreeController.getInstance().getPort2dTree(), "DHBN", BuildTreeController.getInstance().getBSTByCallSign(), new SimpleDateFormat("dd/MM/yyyy HH:mm").parse("31/12/2020 01:25")));
        }
    }
    ...
    void getNearestPort() throws ParseException {
        ...
        if (!ReadFilesController.isUseBigFiles()) {
            ...
            Port p = new Port("America", "United States", 14635, "Los Angeles", 33.717, -118.267);
            Ship ship1 = new Ship(211331640,new SimpleDateFormat("dd/MM/yyyy HH:mm").parse("31/12/2020 01:25"),36.39094,-122.71335,19.7,145.5,147,"SEOUL EXPRESS","IMO9193305","DHBN",70,294,32,13.6,79,"B","12345");
            ShipCallSign shipCallSign = new ShipCallSign(ship1);
            Map<String, ShipCallSign> map = new HashMap<>();
    ...
    void getNearestPortNoCallsign() throws ParseException {
        ...
        if (!ReadFilesController.isUseBigFiles()) {
            ...
            Ship ship1 = new Ship(211331640,new SimpleDateFormat("dd/MM/yyyy HH:mm").parse("31/12/2020 01:25"),36.39094,-122.71335,19.7,145.5,147,"SEOUL EXPRESS","IMO9193305","DHBN",70,294,32,13.6,79,"B","32454");
            ShipCallSign shipCallSign = new ShipCallSign(ship1);
            Map<String, ShipCallSign> map = new HashMap<>();
            List<Map.Entry<String, ShipCallSign>> groupedList = new LinkedList<>(map.entrySet());

            assertNull(findClosestPort.getNearestPort(BuildTreeController.getInstance().getPort2dTree(), groupedList, "DHBN", new SimpleDateFormat("dd/MM/yyyy HH:mm").parse("31/12/2020 01:25")));
        }
    }
